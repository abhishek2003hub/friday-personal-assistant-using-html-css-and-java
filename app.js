...
const friday_intro = document.querySelector("#j_intro");
...
// 
// start friday with btn
document.querySelector("#start_friday_btn").addEventListener("click", () => {
  recognition.start();
})


document.querySelector("#stop_friday_btn").addEventListener("click", () => {
  stopingR = true;
  recognition.stop();
})
...
recognition.onstart = function () {
  ...
  document.querySelector("#stop_friday_btn").style.display = "flex"
};
...
recognition.onresult = function (event) {
  ...
  if(localStorage.getItem("lang") === "en-US"){
    if (transcript.includes("hi friday")) {
      readOut("hello sir");
    }
    ...
    if (transcript.includes("Tell about yourself")) {
      readOut( "sir, i am a friday, a voice asistant made for browsers using javascript by one of the Enthusiastic dev on the planet. I can do anything which can be done from a browser."
      );
    }
    ...
    if (transcript.includes("full weather report")) {
      ...
      let a = window.open(
        `https://www.google.com/search?q=weather+in+${
          JSON.parse(localStorage.getItem("friday_setup")).location
        }`
      );
      windowsB.push(a)
    }
...
recognition.onend = function () {
  if (stopingR === false) {
    ...
    recognition.stop();
    document.querySelector("#stop_friday_btn").style.display = "none"
  }
};
...
//   friday
const smallfriday = document.querySelector("#small_friday")



smallfriday.addEventListener("click", () => {
...
document.querySelector("#friday_start").addEventListener("click", () => {
  recognition.start()
})
